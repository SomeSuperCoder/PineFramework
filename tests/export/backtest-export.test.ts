...
describe('backtest-export lib — round-trip fidelity', () => {
  ...
  it('serialized JSON uses __nonfinite tags and never emits raw NaN/Infinity', () => {
  ...
  it('a fully-finite export round-trips to a deep-equal object', () => {
  ...
  it('preserves a null/na value in a series as JSON null (never coerced to 0)', () => {
    const ctx = makeContext({
      output: {
        series: new Map<string, Series>([
  ...
  it('copies engine-owned arrays — mutating inputs after build does not alias the export', () => {
...
describe('backtest-export lib — effectiveConfig guard', () => {
  it('THROWS when params.effectiveConfig is unavailable', () => {
    const ctx = makeContext();
...
describe('backtest-export lib — meta and fingerprint defaults', () => {
  it('barCount defaults to bars.length when omitted; explicit startDate/endDate are included', () => {
    const built = buildBacktestExport(
      makeContext({
  ...
  it('uses a precomputed fingerprint when provided', () => {
    const built = buildBacktestExport(
...
describe('backtest-export lib — computeInputFingerprint', () => {
  ...
  it('is deterministic for identical bars', () => {
    expect(computeInputFingerprint(bars)).toBe(computeInputFingerprint(makeBars()));
  });

  it('changes when a bar value changes (input drift detection)', () => {
...
describe('backtest-export lib — exportFilename', () => {
  it('formats backtest-<source>-<symbol>-<runId>-<ISO-timestamp>.json', () => {
  ...
  it('replaces ":" and "." in the ISO timestamp with "-" so the name is filesystem-safe', () => {
  ...
  it('embeds the runId and the ISO timestamp — its parts reconstruct the original ms', () => {
