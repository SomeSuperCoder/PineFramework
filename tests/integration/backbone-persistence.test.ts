...
describe('S/R backbone persistence across ticks', () => {
  ...
  it('backbone produces lines during init and continues through ticks', () => {
    ...
    for (const l of initLines) {
      ...
      console.log(
        `  Line entry: x1=${entry.x1} y1=${entry.y1?.toFixed(2)} x2=${entry.x2} y2=${entry.y2?.toFixed(2)} xloc=${entry.xloc}`,
      );
    }
    ...
    for (const l of tick1Result.diffLines) {
      console.log(
        `  Line: x1=${l.x1} y1=${l.y1?.toFixed(2)} x2=${l.x2} y2=${l.y2?.toFixed(2)} xloc=${l.xloc}`,
      );
    }
