...
describe('HHLL End-to-End Pivot Test', () => {
  ...
  it('debugs pivot detection directly', () => {
    // Test pivothigh/pivotlow directly with the controlled data
    const testSrc = `//@version=6
indicator("PivotTest")
plot(ta.pivothigh(5, 5), "ph")
plot(ta.pivotlow(5, 5), "pl")`;

    const { ast } = parse(testSrc);
    const compiled = compile(ast);
    const engine = new ExecutionEngine(compiled);

    // Need 5+5+1 = 11 bars min. Pivot at bar 4 confirmed at bar 10 (4+5+1 for len≥11)
    const bars: Array<{ timestamp: number; open: number; high: number; low: number; close: number; volume: number }> = [];
    const ts = 1700000000000;
    ...
    // Bar 4 pivot high: bars -5..-1 (0..3) and +1..+5 (5..9) must be LOWER than bar 4
    // Bars 0..3 are gradually increasing but stay below bar 4
    bars[0].high = 100;
    bars[1].high = 105;
    bars[2].high = 110;
    bars[3].high = 115;
    bars[4].high = 130; // PEAK
    // Bars 5..9 decline
    bars[5].high = 125;
    bars[6].high = 123;
    bars[7].high = 120;
    ...
    // Find the ph and pl output keys
    ...
    // Pivot at bar 4 is confirmed at bar 9 (p + rightBars = 4 + 5).
    // At bar 9 (len=10), candidate is getRelative(5)=bar[4].high=130.
    // All right-side bars (5-9) and left-side bars (0-3, missing -1 is NA) pass.
    ...
    console.log(`\nph[9] = ${phValues[9]} (expected 130) — pivot at bar 4 confirmed at p+rightBars`);
    console.log(`ph[10] = ${phValues[10]} (expected null — bar 5 is not a pivot)`);
    expect(phValues[9]).toBe(130);
  });
