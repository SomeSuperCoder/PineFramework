...
export function executeArrayMethod(
  ...
  switch (methodName) {
    ...
    case 'stdev': {
      ...
      const variance =
        nums.reduce((sum: number, v: number) => sum + (v - mean) ** 2, 0) / (nums.length - 1);
      return Math.sqrt(variance);
    }
    case 'indexof':
      for (let idx = 0; idx < obj.length; idx++) if (obj[idx] === args[0]) return idx;
      return -1;
    case 'clear':
      obj.length = 0;
      return NA;
    case 'percentile_linear_interpolation': {
      ...
      const nums = obj
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .filter((v: any): v is number => typeof v === 'number' && !isNaN(v) && Number.isFinite(v))
    ...
    case 'copy':
      return [...obj];
    default:
      return undefined; // not an array method
  }
}
