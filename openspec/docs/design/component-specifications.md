...
- **Telegram Integration (Telegraf Bot)**:
  ...
  - **HTTP Proxy**: All Telegram Bot API outbound connections are routed through a configurable HTTP proxy
...
- **HTTP Proxy**: All Telegram Bot API outbound connections are routed through an HTTP proxy when configured
  - Proxy settings (host, port, username, password) stored in the `proxy` key under `settings` in `backend/data/telegram.json`
  - On initialization, if proxy settings are present, an HTTP proxy agent is created and configured as the Telegram HTTP agent
