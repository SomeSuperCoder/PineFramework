## Purpose
Support the offset parameter in barcolor() to shift coloring forward or backward by a specified number of bars.

## Requirements

### Requirement: barcolor() offset parameter
The engine SHALL support the `offset` parameter in `barcolor()` to shift the coloring.
