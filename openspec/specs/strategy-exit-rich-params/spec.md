## Purpose
Support advanced strategy.exit() parameters including profit/loss ticks, qty percent, and from_entry targeting.

## Requirements

### Requirement: Profit/Loss Ticks Parameters
The `strategy.exit()` builtin SHALL accept `profit` and `loss` parameters specified in ticks from the entry price.

#### Scenario: Profit parameter sets limit at entry + profit ticks
- **WHEN** strategy.exit("TP", profit=100) is called on a long entry at $100 with mintick=0.01
- **THEN** the engine SHALL place a limit exit order at $101.00

#### Scenario: Loss parameter sets stop at entry - loss ticks
- **WHEN** strategy.exit("SL", loss=50) is called on a long entry at $100 with mintick=0.01
- **THEN** the engine SHALL place a stop exit order at $99.50
...
- **THEN** the engine SHALL place an exit order for only 30 units
