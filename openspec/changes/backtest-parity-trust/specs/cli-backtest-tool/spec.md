...
Align the CLI backtest tool with the parity and trust contract: require an explicit official commission method (no silent 0-commission default), resolve date ranges with the same UTC-midnight semantics as other producers, and show the user what actually ran (effective config summary and warnings) in the CLI output.
