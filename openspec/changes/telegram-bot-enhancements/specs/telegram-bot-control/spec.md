...
- Control data lives in `telegram.json` under `admin`, `controllers`, and `requests` keys.
- Every privileged handler gate is `assertController(fromId)` (admin OR controller); admin always acts.
- No privileged decision is ever based on username.